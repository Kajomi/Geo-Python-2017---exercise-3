...
for num in range(len(stations)):
    ...
    # Add your code here
    if (latitude > NScutoff) and (longitude < EWcutoff):
        north_west.append(name)
    elif (latitude > NScutoff) and (longitude > EWcutoff):
...
nw_share = len(north_west) / len(stations) * 100 
ne_share = len(north_east) / len(stations) * 100
sw_share = len(south_west) / len(stations) * 100
se_share = len(south_east) / len(stations) * 100
